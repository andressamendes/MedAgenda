...
- **RLS (Row Level Security)**: a tabela opcional `ai_metrics` (`sql/08_ai_metrics.sql`) tem RLS habilitada, com uma política que permite a cada usuário `select` apenas suas próprias linhas (`auth.uid() = user_id`); não há política de `insert` para o papel anônimo, de modo que apenas o papel de serviço poderia gravar métricas nela.
...
- **Tabela `ai_metrics`**: definida em `sql/08_ai_metrics.sql`, com RLS e política de leitura por usuário. A Edge Function, no entanto, apenas registra métricas via `console.log` (`[ai-chat] type=... user=... status=... ms=...`); não há nenhuma instrução de `insert` na tabela `ai_metrics` dentro de `index.ts`. A tabela existe no esquema do banco, mas não é alimentada pelo código atual.
...
- **Integrações:** autenticação via Supabase Auth (JWT), leitura de eventos via `getEvents()`/`recurrence.js`, e uma tabela opcional de métricas (`ai_metrics`) presente no esquema mas não alimentada pelo código atual.
- **Avaliação geral:** o módulo cumpre o princípio de isolamento de chave e responsabilidade única (`aiService.js` como gateway) de forma consistente. As lacunas identificadas na auditoria (ausência de timeout no servidor, ausência de escrita em `ai_metrics`, sobreposição de tratamento de erros `401`/`403`) são limitadas em escopo e não comprometem a segurança da chave de API, que permanece corretamente confinada ao backend.
