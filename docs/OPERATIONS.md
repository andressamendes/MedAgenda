...
4. Confere que todas as tabelas obrigatórias (`REQUIRED_TABLES`) e as colunas críticas introduzidas por migrations recentes (`REQUIRED_COLUMNS` — hoje, `profiles.daily_goal_minutes`/`weekly_goal_minutes`/`monthly_goal_minutes`, de `12_time_goals.sql`) existem, consultando recurso por recurso via REST (`GET {SUPABASE_URL}/rest/v1/<tabela>?select=<coluna>&limit=1`) — o mesmo formato de requisição já usado para `schema_version`, com a anon key. **Não** usa a introspecção OpenAPI da raiz da API (`GET {SUPABASE_URL}/rest/v1/`): esse caminho retorna HTTP 401 em produção pelo gateway do Supabase (só rotas por recurso são expostas publicamente, não a raiz) — descoberto rodando o script contra o projeto real. `PGRST205` (relation not found) identifica tabela ausente; `42703` (undefined_column) identifica coluna ausente — nunca confundidos com qualquer outro erro HTTP (ex.: 401/500), que é reportado como "não foi possível consultar", não como "ausente".
