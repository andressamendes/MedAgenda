...
- **Tabela `ai_metrics` criada mas não alimentada.** A migration `08_ai_metrics.sql` e sua política de RLS existem, mas nenhuma Edge Function ou código do frontend insere dados nela — a única telemetria de IA existente hoje é `console.log` na Edge Function `ai-chat`.
