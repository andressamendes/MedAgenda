...
- **Observação:** a função registra `console.log`/`console.error` com tipo de prompt, usuário, status HTTP e latência, e também persiste uma linha por chamada (sucesso ou falha) na tabela `ai_metrics` via `service_role`, em background (`EdgeRuntime.waitUntil`) — sem armazenar prompt, resposta ou dados pessoais (ver seção "Auditoria Arquitetural").
...
O banco é um **PostgreSQL** gerenciado pelo Supabase, com 13 tabelas de domínio: as 8 originais de Planejamento (`events`, `categories`, `profiles`, `push_subscriptions`, `notification_logs`, `academic_calendars`, `academic_events`, `ai_metrics`) mais 5 introduzidas a partir da migration `11` para o domínio de Execução de Estudo (`activity_sessions`, `reviews`, `questions`, `reflections`, `schema_version`). Todas as tabelas de usuário têm **RLS habilitado** e políticas baseadas em `auth.uid()` (diretamente ou via `EXISTS` para tabelas filhas como `academic_events`). Há uma função e trigger compartilhados (`update_updated_at`) para manter `updated_at` sincronizado, e índices compostos para as consultas mais frequentes (por `user_id` + data). O domínio de Execução (Sessão, Questões, Revisões, Reflexão) e o Session Event Bus que o conecta às Views estão documentados em [`ARCHITECTURE.md`](ARCHITECTURE.md); o schema completo das 20 migrations está em [`DATABASE.md`](DATABASE.md).
...
- **Métricas de IA (`ai_metrics`):** tabela dedicada no banco para registrar `prompt_type`, `model`, `duration_ms`, `success`, `http_status`, `error_code` e `error_message` por chamada de IA. A Edge Function `ai-chat` insere uma linha por chamada (sucesso ou falha), via `service_role`, sem armazenar prompt, resposta da IA ou dados pessoais — ver "Auditoria Arquitetural".
